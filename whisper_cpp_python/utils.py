...
def get_file_path(model_name: str, download_dir: str | None = None) -> Path:
    ...
    if download_dir is None:
        ...
        logger.info(
            "No download directory was provided, "
            f"models will be downloaded to {download_dir}"
        )
...
def download_model(model_name: str, download_dir=None, chunk_size=1024) -> str:
    ...
    file_path = get_file_path(model_name, download_dir=download_dir)
    if file_path.exists():
...
def output_vtt(segments: list, output_file_path: str) -> str:
    ...
    with open(absolute_path, "w") as file:
        ...
        for seg in segments:
            file.write(
                "{} --> {}\n".format(
                    to_timestamp(seg.t0, separator="."),
                    to_timestamp(seg.t1, separator="."),
                )
            )
...
def redirect_stderr(to: bool | TextIO | str | None = False) -> None | Generator:
    """
    ...
    def _resolve_target(target: "FileDescriptorOrPath" | None) -> tuple[TextIO, bool]:
        ...
        if hasattr(target, "write"):
            return target, False
        raise ValueError(
            "Invalid `to` parameter; expected None, "
            "a filepath string, or a file-like object."
        )
